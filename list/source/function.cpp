...
void Skaicius1(list<studentas>& s, int &m, int skaicius, int MinP, int MaxP)
{
  ...
  cout<<"Enter the number of students: "; m=intIvestis2();
      ...
      for(int i=0; i<m; i++)
      {
       ...
       st.nd.sort();
            st.ndCount = n;
            auto nd_vector = vector<double>(st.nd.begin(), st.nd.end());

            if (n % 2 != 0) {
                st.median = nd_vector[n / 2];
            } else {
                st.median = (nd_vector[n / 2 - 1] + nd_vector[n / 2]) / 2.0;
            }
  ...
  while(true)
      {
        ...
        st.nd.sort();
            ...
            if (st.ndCount % 2 != 0) {
                st.median = nd_vector[st.ndCount / 2];
            } else {
                st.median = (nd_vector[st.ndCount / 2 - 1] + nd_vector[st.ndCount / 2]) / 2.0;
            }
...
void Sorting(list<studentas>& s, list<studentas> &k, list <studentas> &l, string choice, int rikiavimas, int strategy) {
    if (rikiavimas == 1) {
      if(choice == "a"){
        ...
        if(it->average < 5.0) {
            l.push_back(*it);  // Pridėk studentą į vargšiukų konteinerį
            it = s.erase(it);  // Ištrink studentą iš bendro konteinerio
        } else {
            ++it;
        }
           }}
         if (strategy == 3) {
          auto partitionPoint = std::partition(s.begin(), s.end(), isBelowAverage);
          l.insert(l.end(), s.begin(), partitionPoint);
  ...
  cout << "Rusiavimas uztruko: " << duration_rusiavimas/1000.0 << " sekundes" << endl;
        ...
        if(strategy==1)
        for(auto stud: s){
                    if(stud.median < 5.0) l.push_back(stud);
                    else k.push_back(stud);
         }
          if(strategy==2){
           for(auto it = s.begin(); it != s.end(); ) {
        if(it->median < 5.0) {
         ...
         if (strategy == 3) {
          auto partitionPoint = std::partition(s.begin(), s.end(), isBelowMedian);
          l.insert(l.end(), s.begin(), partitionPoint);
          s.erase(s.begin(), partitionPoint); 
            }
